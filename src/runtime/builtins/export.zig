...
fn runExport(state: *State, argv: []const []const u8) u8 {
...
test "Export: syntax variants" {
    // NAME=VALUE syntax
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_TEST_VAR=testvalue" }));
    ...
    // NAME=VALUE with empty value
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_EMPTY=" }));
    ...
    // Multiple NAME=VALUE pairs
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_MULTI_A=aval", "OSHEN_MULTI_B=bval" }));
    ...
    // Space-separated NAME VALUE syntax
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_SPACE_VAR", "spacevalue" }));
...
test "Export: existing variables and overwriting" {
    // Export existing shell variable
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_SHELLVAR" }));
    ...
    // Overwriting existing export
    {
        ...
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_OVERWRITE=first" }));
        try expectExported(&state, "OSHEN_OVERWRITE", "first");
        try testing.expectEqual(@as(u8, 0), runExport(&state, &.{ "export", "OSHEN_OVERWRITE=second" }));
...
test "Export: error cases" {
    const cases = .{
        .{ &[_][]const u8{ "export", "OSHEN_NONEXISTENT_67890" }, @as(u8, 1) }, // nonexistent variable
        .{ &[_][]const u8{ "export", "OSHEN_TOO_MANY", "value", "extra" }, @as(u8, 1) }, // too many arguments
    };
