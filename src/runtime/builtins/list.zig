...
pub const builtin = builtins.Builtin{
    ...
    \\  @VARNAME         Read list variable directly (efficient for large lists)
    ...
    \\  --into VARNAME   Write result to variable instead of stdout
    \\  --inplace        Write result back to source variable (requires @VARNAME)
    \\
    ...
    \\  list @items --sort --into sorted  # sort variable, store in another
    \\  list @items --sort --inplace      # sort variable in place
    \\  list @items --contains foo        # exit 0 if foo in items
    \\  list @items --length              # count items
    \\  list @items --append x y z        # append items
    \\  list a b c -S ', '                # a, b, c
    ,
};
...
const Config = struct {
    ...
    // Source variable (set when using @varname, used by --inplace)
...
fn run(state: *builtins.State, cmd: builtins.ExpandedCommand) u8 {
    var cfg = Config{};
    var input_source: InputSource = .{ .items = &.{} };

    // Parse arguments
    if (!parseArgs(state, cmd.argv, &cfg, &input_source)) {
        return 1;
    }
    ...
    // Collect main input
    collectInput(state, &input_source, &cfg, &items) catch |err| switch (err) {
    ...
    return executeMode(state, result, &cfg);
}
...
fn parseArgs(state: *builtins.State, argv: []const []const u8, cfg: *Config, input: *InputSource) bool {
    _ = state;
    var p = args.Args("list").init(argv);

    // Track indices for slicing argv later
    ...
    while (p.next()) |arg| {
        ...
        if (p.option("-s", "--step")) |val| {
            ...
            cfg.separator = val;
        } else if (p.option(null, "--into")) |val| {
            cfg.into_var = val;
    ...
    // Set slices from argv
    if (append_end > append_start) {
        cfg.append_items = argv[append_start..append_end];
    }
    if (prepend_end > prepend_start) {
        cfg.prepend_items = argv[prepend_start..prepend_end];
    }

    // Determine input source from positional args
    const positionals = if (positional_end > positional_start) argv[positional_start..positional_end] else &[_][]const u8{};
    if (positionals.len == 0) {
        input.* = .stdin;
    } else if (positionals.len == 1 and positionals[0].len > 1 and positionals[0][0] == '@') {
        const varname = positionals[0][1..];
        input.* = .{ .variable = varname };
        cfg.source_var = varname;
    } else {
        input.* = .{ .items = positionals };
    }

    // Validate --inplace requires @varname
    if (cfg.inplace) {
        if (cfg.source_var == null) {
            io.writeStderr("list: --inplace requires @VARNAME input\n");
            return false;
        }
        if (cfg.into_var != null) {
            io.writeStderr("list: --inplace cannot be used with --into\n");
            return false;
        }
        cfg.into_var = cfg.source_var;
    }
...
fn collectInput(state: *builtins.State, input: *const InputSource, cfg: *const Config, items: *std.ArrayListUnmanaged([]const u8)) CollectError!void {
    const allocator = state.allocator;
    switch (input.*) {
        .items => |raw_items| {
            for (raw_items) |item| {
                try addItem(allocator, item, cfg, items);
            }
        },
        .variable => |varname| {
            // Try to get as list first (efficient path)
            if (state.getVarList(varname)) |list| {
                try items.appendSlice(allocator, list);
            } else if (state.getVar(varname)) |scalar| {
                // Scalar variable - treat as single item
                try items.append(allocator, scalar);
            } else {
                io.writeStderr("list: variable '");
...
fn addItem(allocator: std.mem.Allocator, item: []const u8, cfg: *const Config, items: *std.ArrayListUnmanaged([]const u8)) !void {
...
fn executeMode(state: *builtins.State, items: []const []const u8, cfg: *const Config) u8 {
    switch (cfg.mode) {
        .contains => {
            for (items) |item| {
        ...
        .length => {
            if (cfg.into_var) |varname| {
                var buf: [21]u8 = undefined;
                const len_str = std.fmt.bufPrint(&buf, "{d}", .{items.len}) catch unreachable;
                state.setVar(varname, len_str) catch return oom();
            } else {
                var w = Writer{};
                w.writeInt(items.len);
                w.writeByte('\n');
                w.flush();
            }
            return 0;
        ...
        .output => {
            if (cfg.into_var) |varname| {
                state.setVarList(varname, items) catch return oom();
            } else {
                var w = Writer{};
                for (items, 0..) |item, i| {
                    if (i > 0) w.write(cfg.separator);
                    w.write(item);
                }
                if (items.len > 0) w.writeByte('\n');
                w.flush();
            }
            return 0;
        },
...
fn runList(state: *State, argv: []const []const u8) u8 {
    return run(state, .{ .argv = argv, .env = &.{}, .redirects = &.{} });
}
...
test "Command: @varname reads list and transforms" {
    ...
    // Test @varname with --length --into
    try state.setVarList("mylist", &.{ "x", "y", "z" });
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@mylist", "--length", "--into", "count" }));
    try testing.expectEqualStrings("3", state.getVar("count").?);

    // Test @varname with --sort --into
    try state.setVarList("unsorted", &.{ "c", "a", "b" });
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@unsorted", "--sort", "--into", "sorted" }));
    try expectList(&state, "sorted", &.{ "a", "b", "c" });
}
...
test "Command: --contains and --empty exit codes" {
    ...
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "a", "b", "c", "--contains", "b" }));
    try testing.expectEqual(@as(u8, 1), runList(&state, &.{ "list", "a", "b", "c", "--contains", "x" }));

    // --empty: empty=0, non-empty=1
    try state.setVarList("empty", &.{});
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@empty", "--empty" }));

    try state.setVarList("nonempty", &.{"x"});
    try testing.expectEqual(@as(u8, 1), runList(&state, &.{ "list", "@nonempty", "--empty" }));
}
...
test "Command: --append and --prepend mutations" {
    ...
    // --append
    try state.setVarList("base1", &.{ "a", "b" });
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@base1", "--append", "c", "d", "--into", "appended" }));
    try expectList(&state, "appended", &.{ "a", "b", "c", "d" });

    // --prepend
    try state.setVarList("base2", &.{ "c", "d" });
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@base2", "--prepend", "a", "b", "--into", "prepended" }));
    try expectList(&state, "prepended", &.{ "a", "b", "c", "d" });
}

test "Command: --inplace mutates source variable" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    var state = State.init(arena.allocator());
    state.initCurrentScope();
    ...
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@items", "--sort", "--inplace" }));
    try expectList(&state, "items", &.{ "a", "b", "c" });

    // --inplace with reverse
    try state.setVarList("nums", &.{ "1", "2", "3" });
    try testing.expectEqual(@as(u8, 0), runList(&state, &.{ "list", "@nums", "--reverse", "--inplace" }));
