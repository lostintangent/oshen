...
pub fn Spec(comptime cmd: []const u8, comptime def: anytype) type {
    ...
    return struct {
        ...
        pub fn parse(argv: []const []const u8) ParseError!Result {
            return ArgsParser.parse(argv, def.args);
        }
...
pub fn Args(comptime cmd: []const u8) type {
    return struct {
        ...
        /// Generate a Result type from a spec and parse argv into it.
        ///
        /// Usage:
        /// ```
        /// const r = Args("range").parse(cmd.argv, .{
        ...
        /// ```
        pub fn parse(argv: []const []const u8, comptime spec: anytype) ParseError!Result(@TypeOf(spec)) {
            var self = Self.init(argv);
        ...
        pub fn next(self: *Self) ?[]const u8 {
            ...
            self.current = self.argv[self.idx];
            self.idx += 1;
            return self.current;
        }

        pub fn peek(self: *const Self) ?[]const u8 {
            return if (self.idx < self.argv.len) self.argv[self.idx] else null;
        }
        ...
        pub fn rest(self: *const Self) []const []const u8 {
            return if (self.idx <= self.argv.len) self.argv[self.idx..] else &.{};
        }
...
test "Args: imperative - basic iteration" {
    const argv = [_][]const u8{ "cmd", "a", "b", "c" };
...
test "Args: imperative - flag matching" {
    const argv = [_][]const u8{ "cmd", "-v", "--verbose", "arg" };
    var p = Args("test").init(&argv);
...
test "Args: imperative - option with value" {
    const argv = [_][]const u8{ "cmd", "--step", "5", "--count=10" };
    var p = Args("test").init(&argv);
...
test "Args: declarative - flags" {
    const argv = [_][]const u8{ "cmd", "-v", "--quiet", "arg1", "arg2" };
...
test "Args: declarative - options" {
    const argv = [_][]const u8{ "cmd", "--step", "5", "-n", "10", "arg" };
...
test "Args: declarative - string options" {
    const argv = [_][]const u8{ "cmd", "--name=hello", "arg" };
...
test "Args: declarative - unknown flag error" {
    const argv = [_][]const u8{ "cmd", "--unknown" };
...
test "Args: declarative - end of flags" {
    const argv = [_][]const u8{ "cmd", "-v", "--", "-not-a-flag" };
...
test "Args: declarative - negative numbers as positional" {
    const argv = [_][]const u8{ "cmd", "-5", "10" };
...
test "Args: peekI64 - valid number" {
    const argv = [_][]const u8{ "cmd", "--flag", "42", "rest" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd)
    _ = p.next(); // consume --flag, idx becomes 2
    try testing.expectEqual(@as(i64, 42), p.peekI64(0));
    try testing.expectEqual(@as(usize, 3), p.idx); // consumed "42"
}

test "Args: peekI64 - negative number" {
    const argv = [_][]const u8{ "cmd", "-5" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), so peek sees "-5"
    try testing.expectEqual(@as(i64, -5), p.peekI64(0));
}

test "Args: peekI64 - not a number returns default" {
    const argv = [_][]const u8{ "cmd", "--next" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), peek sees "--next" which is not numeric
    try testing.expectEqual(@as(i64, 99), p.peekI64(99));
    try testing.expectEqual(@as(usize, 1), p.idx); // not consumed
}

test "Args: peekI64 - no next arg returns default" {
    const argv = [_][]const u8{"cmd"};
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), no more args
    try testing.expectEqual(@as(i64, 99), p.peekI64(99));
}

test "Args: peekUsize - valid positive number" {
    const argv = [_][]const u8{ "cmd", "42" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), peek sees "42"
...
test "Args: peekUsize - negative returns default" {
    const argv = [_][]const u8{ "cmd", "-5" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), peek sees "-5" which is negative
    try testing.expectEqual(@as(usize, 99), p.peekUsize(99));
    try testing.expectEqual(@as(usize, 1), p.idx); // not consumed
}

test "Args: peekString - valid non-flag" {
    const argv = [_][]const u8{ "cmd", "hello" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), peek sees "hello"
    try testing.expectEqualStrings("hello", p.peekString().?);
}

test "Args: peekString - flag returns null" {
    const argv = [_][]const u8{ "cmd", "--flag" };
    var p = Args("test").init(&argv);
    // idx starts at 1 (skipping cmd), peek sees "--flag" which looks like a flag
...
test "Args: typed positionals - too many" {
    const argv = [_][]const u8{ "cmd", "1", "2", "3" };
...
test "Args: typed positionals - with options" {
    const argv = [_][]const u8{ "cmd", "--step", "2", "5" };
...
test "Args: typed positionals - with options and both args" {
    const argv = [_][]const u8{ "cmd", "-s", "2", "3", "10" };
...
test "Args: Rest - with flags" {
    const argv = [_][]const u8{ "cmd", "-n", "hello", "world" };
